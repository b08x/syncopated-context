...
description: "Ruby project scaffolder using rubysmith (apps, tools, scripts) or gemsmith (publishable gems). Activates on any mention of: scaffold a project, scaffold a gem, new ruby project, new gem, create gem, bootstrap project, rubysmith, gemsmith, generate project skeleton, project template, set up a new ruby app, start a ruby tool, start a ruby script, create a new gem for rubygems, initialize ruby project, rubysmith build, gemsmith build, new ruby app, create a ruby tool. Runs rubysmith for local Ruby tools and apps; runs gemsmith for gems intended for rubygems.org. Executes the CLI, displays generated file tree, then optionally applies Standard Mode convention hardening. After scaffolding, suggests and chains to rubysmithing-tui, rubysmithing-genai, rubysmithing-yardoc, and rubysmithing-context. Use whenever a user wants to start a new Ruby project from scratch."
---
