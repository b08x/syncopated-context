...
Usage:
  ...
  python3 resolve_citations.py --qa /tmp/qa.json --sources /tmp/sources.json \
    ...
    --output "NotebookLM/my-notebook/QA/2026-02-23 My Q&A.md"
...
def resolve_answer(
    ...
    sources_path = f"NotebookLM/{slug}/Sources"
...
def main():
    parser = argparse.ArgumentParser(
        description="Resolve NotebookLM citations to wikilinks"
    )
    parser.add_argument(
        "--qa", required=True, help="Path to notebooklm ask --json output"
    )
    parser.add_argument(
        "--sources", required=True, help="Path to notebooklm source list --json output"
    )
    ...
    parser.add_argument(
        "--dashboard", help="Dashboard title for related links (required for --output)"
    )
    parser.add_argument(
        "--output", help="Vault-relative output path for reference note"
    )
    parser.add_argument(
        "--passages",
        help="Passage mapping JSON from extract_passages.py (enables #Passage N anchors)",
    )
    ...
    if passage_map:
        print(
            f"Passage anchors: {stats['passage_hits']} linked, {stats['passage_misses']} source-only",
            file=sys.stderr,
        )
    if stats["uncited_sources"]:
        print(
            f"WARNING: {len(stats['uncited_sources'])} sources have no citations in the answer:",
            file=sys.stderr,
        )
    ...
    if not args.title or not args.dashboard:
        print(
            "ERROR: --title and --dashboard required when using --output",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    sources_path = f"NotebookLM/{args.slug}/Sources"
    ...
    if stats["uncited_sources"]:
        uncited_links = "\n".join(
            f"- [[{sources_path}/{s}|{s}]]" for s in stats["uncited_sources"]
        )
        uncited_section = f"""
...
{uncited_section}"""
    ...
    print(
        f"CREATED: {args.output} ({len(resolved)} chars, {stats['cited_sources']}/{stats['total_sources']} sources cited)"
    )
